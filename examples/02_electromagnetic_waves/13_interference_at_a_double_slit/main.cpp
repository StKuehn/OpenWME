...
int main()
{
	...
	for (int i = -5; i <= 5; i++)
	{
		...
		HertzianDipole->SetLinearTrajectory(TVector(-0.5 * m, 0, i * 10 * cm), TVector(0, 0, 0));
	...
	for (int iz = -80; iz <= 80; iz++)
	{
		...
		Reflector->SetLinearTrajectory(TVector(0, 0, zv), TVector(0, 0, 0));
